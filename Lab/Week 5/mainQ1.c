...
int palindrome(Queue *wordPtr) {
// initialise char*
    char *word = malloc(sizeof(char) * (wordPtr->size + 1));
    char *reverseWord = malloc(sizeof(char) * (wordPtr->size + 1));
    int i = 0;
    int i2 = 0;
...
// loop through the queue and print
    QueueNode *temp = wordPtr->head;
    while (temp != NULL) {
        if (temp->item != ' ') {
            char tem = temp->item;
            if (temp->item >= 'A' && temp->item <= 'Z') {
                tem = temp->item + 32;
            }
            word[i] = tem;
            i++;
